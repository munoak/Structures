...
bool change_phone_number(const std::string &student_number,
                        std::map< std::string, Student* >& students_by_number) {
  // First check student existanne in list
  if (students_by_number.find(student_number) == students_by_number.end() )
    {
  ...
  Student *student = students_by_number.at(student_number);
...
void store_student_data (const std::string& file_name,
                         std::map< std::string, Student* >& students_by_id){

  // open file
  std::ofstream data_file(file_name);
  if( data_file )
    {
      std::map<std::string, Student*>::const_iterator iter = students_by_id.begin();
      while (iter != students_by_id.end())
        {
          Student *s = iter->second;
            data_file << s->student_number << ';'
                      << s->user_id << ';'
                      << s->name << ';'
                      << s->phone_number << ';'
                      << s->email << ';'
...
int main() {
  ...
  while(true) {
      ...
      if(command == "U" or command == "u") {
          ...
          if (change_phone_number(parts.at(1), student_numbers))
            {
              store_student_data(file_name, user_ids);
            }
